...
"""
This file contains primitives for multi-mlu communication.
...
def get_num_devices():
    mlu_list = os.getenv('MLU_VISIBLE_DEVICES', None)
    if mlu_list is not None:
        return len(mlu_list.split(','))
    else:
        devices_list_info = os.popen("cnmon")
...
def _serialize_to_tensor(data, group):
    ...
    assert backend in ["gloo", "cncl"]
    device = torch.device("cpu" if backend == "gloo" else "mlu")
