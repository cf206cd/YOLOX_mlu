...
class VOCEvaluator:
    ...
    def evaluate(
        ...
        tensor_type = torch.mlu.HalfTensor if half else torch.mlu.FloatTensor
        model = model.eval()
        if half:
            model = model.half()
        ids = []
        ...
        if trt_file is not None:
            ...
            x = torch.ones(1, 3, test_size[0], test_size[1]).mlu()
            model(x)
            model = model_trt
        ...
        statistics = torch.mlu.FloatTensor([inference_time, nms_time, n_samples])
        if distributed:
